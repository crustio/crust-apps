...
function UnLockingCsm ({ className = '', account, isHighlight, isPadded = true, value, withShrink = false }: Props): React.ReactElement<Props> | null {
  const { api } = useApi();
  const { t } = useTranslation();
  const [unLockings, setUnLockings] = useState<UnLockingMapping[]>([]);
  const csmLedger = useCall<any>(api.query.csmLocking.ledger, [account])
    ...
    <div className={`ui--AddressMini${isHighlight ? ' isHighlight' : ''}${isPadded ? ' padded' : ''}${withShrink ? ' withShrink' : ''} ${className}`}>
      <div className='ui--AddressMini-balances'>
        <React.Fragment >
          ...
          >
            <Tooltip
              text={unLockings.map(({ bn, value }, index): React.ReactNode => (
                <div key={index}>
                  <span style={{"wordWrap": "break-word", "wordBreak": "break-all"}}><FormatCsmBalance value={value} />{t<string>(' will unLocked at block {{bn}}', { replace: { bn } })}</span>
                </div>
              ))}
...
export default React.memo(styled(UnLockingCsm)`
