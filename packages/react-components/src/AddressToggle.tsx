...
function AddressToggle ({ address, className = '', filter, isHidden, noToggle, onChange, targets, value }: Props): React.ReactElement<Props> | null {
  const { api } = useApi();
  ...
  const apy = useMemo(
    () => targets ? getApy(address) : undefined,
    [address, targets]
  );
