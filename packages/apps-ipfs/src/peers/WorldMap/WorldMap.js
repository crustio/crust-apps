...
const WorldMap = ({ className, doSetSelectedPeers, selectedPeers, t }) => {
    ...
    <div className='flex flex-column'>
      <div className={`relative ${className}`}>
        <div className='mb1 flex flex-column items-center'>
          <div className='relative no-events'
            style={{ width, height, background: `transparent url(${staticMapSrc}) center no-repeat`, backgroundSize: 'auto 100%' }}>
            <GeoPath height={height}
              width={width}>
              { ({ path }) => (
                <MapPins handleMouseEnter={ handleMapPinMouseEnter }
      ...
      <div className='relative flex justify-end pt2 pb4'>
        <div className='f6 p2 no-select flex items-center'>
          ...
          <i className='mapDotExplanation mr1'
            style={{ width: getDotsSize(1) * 2, height: getDotsSize(1) * 2, backgroundColor: getDotsColor(1) }}></i>1-10
          <i className='mapDotExplanation ml3 mr1'
            style={{ width: getDotsSize(50) * 2, height: getDotsSize(50) * 2, backgroundColor: getDotsColor(50) }}></i> 10-100
          <i className='mapDotExplanation ml3 mr1'
            style={{ width: getDotsSize(110) * 2, height: getDotsSize(110) * 2, backgroundColor: getDotsColor(110) }}></i>100+
        </div>
      </div>
    </div>
  );
...
const GeoPath = ({ children, height, width }) => {
...
const MapPins = connect('selectPeersCoordinates', ({ handleMouseEnter, handleMouseLeave, height, path, peersCoordinates, width }) => {
  const el = d3.select(ReactFauxDOM.createElement('svg'))
    .attr('width', width)
    .attr('height', height)
    .attr('viewBox', `0 0 ${width} ${height}`);

  peersCoordinates.forEach(({ coordinates, peerIds }) => {
    el.append('path')
      .datum({
        type: 'Point',
    ...
    <div className='f6 flex flex-column justify-center'>
      { peers.sort((a, b) => a.address.localeCompare(b.address)).map((peer, index) => {
        if (index === MAX_PEERS && peers.length > MAX_PEERS) {
          return (<div className='f7 pa1 self-end'
            key='worldmap-more-label'>{t('plusPeers', { number: peers.length - MAX_PEERS })}</div>);
        }

        if (index > MAX_PEERS) return null;

        return (
          <div className='pa2 f7 flex items-center monospace'
            key={peer.peerId}>
            <CountryFlag className='mr1'
              code={peer.flagCode}
              svg={isWindows}/>
