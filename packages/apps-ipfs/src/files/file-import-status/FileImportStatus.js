...
const viewImportStatus = (job) => {
  switch (job.status) {
    ...
    case 'Failed': {
      return (<GlyphCancel className='dark-red w2 ph1'
        fill='currentColor'/>);
    }
...
const LoadingIndicator = ({ complete }) => (
  <>
    ...
    { complete && <GlyphTick className='green w2 ph1'
      fill='currentColor'/>}
  </>
);

const FileImportStatus = ({ doFilesClear, filesErrors, filesFinished, filesPending, t }) => {
  ...
  return (
    <div className='fileImportStatus fixed bottom-1 w-100 flex justify-center'
      ...
      <div className='br1 dark-gray w-40 center ba b--light-gray bg-white'
        style={{ pointerEvents: 'auto' }}>
        <div className='fileImportStatusButton pv2 ph3 relative flex items-center no-select pointer charcoal w-100 justify-between'
          ...
          <div className='flex items-center'>
            <button aria-expanded={expanded}
              ...
              className='fileImportStatusArrow'
              onClick={() => setExpanded(!expanded)}>
              <GlyphSmallArrows aria-hidden='true'
                className='w-100'
                fill='currentColor'
                opacity='0.7'/>
            </button>
            <button aria-label={ t('filesImportStatus.closeDropdown') }
              className='fileImportStatusCancel'
              onClick={ handleImportStatusClose }>
              <GlyphSmallCancel className='w-100'
                fill='currentColor'
                opacity='0.7'/>
            </button>
          </div>
        ...
        <ul aria-hidden={!expanded}
          className='fileImportStatusRow pa0 ma0'>
