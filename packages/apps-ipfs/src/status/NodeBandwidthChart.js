...
const Tooltip = ({ bw, pos, show, t }) => {
  if (!show) {
    return null;
  }

  return (
    <div className='sans-serif absolute bg-white pa2 br3'
      style={{
...
class NodeBandwidthChart extends React.Component {
  ...
  getTooltip = (t) => {
    ...
    ReactDOM.render(<Tooltip show={false}
      t={t} />, tooltip);
  ...
  data = () => {
    const { nodeBandwidthChartData, t } = this.props;
  ...
  render () {
    const { animatedPoints, nodeBandwidthChartData, t } = this.props;
