...
function ElrondBackAssets({ className = "" }: Props): React.ReactElement<Props> {
  ...
  const submit = async () => {
    try {
      ...
      if (result) {
        ...
        setResult("Please install MultiversX DeFi Wallet extension first");
        setStatus("error");
        setStatusOpen(true);
      }
  ...
  return (
    <div className={className}>
      <Columar>
        <Columar.Column>
          <HttpStatus isStatusOpen={statusOpen} message={result} setStatusOpen={setStatusOpen} status={status} />
          <Card withBottomMargin>
            <Modal.Content>
              <Banner type="warning">
                <p>
                  {t<string>(
                    "There may be a delay (max to 48 hours) in the arrival of the account. Please note that the transfer amount must be greater than 0"
                  )}
                </p>
              </Banner>
            ...
            <div style={{display: "flex", alignItems: 'center'}}>
              ...
              <Banner type="warning">
                <p>
                  {t<string>("If you do not have MultiversX DeFi Wallet extension installed, please install it first.")}
                  &nbsp;
                  <a
                    ...
                    target="_blank"
                  >
                    {t<string>("Install now...")}
                  </a>
                </p>
              </Banner>
              <h3>
                <span style={{ fontWeight: "bold" }}>{t<string>("Receive Account")}</span>
              </h3>
              <div style={{ display: "flex" }}>
                <img
                  style={{ width: "64px", height: "64px", padding: "1px", verticalAlign: "middle" }}
                  src={logoCrust as string}
                />
                <div style={{ flex: 1, verticalAlign: "middle" }}>
                  <InputAddress
                    ...
                    labelExtra={<Available label={t("transferable")} params={receiveId} />}
                  />
                </div>
              </div>
              <h3>
                <span style={{ fontWeight: "bold" }}>{t<string>("Amount")}</span>
              </h3>
              <div style={{ display: "flex", alignItems: "center" }}>
                  ...
                  {elrondAddress && elrondTxStatus && elrondTxStatus == "success" && elrondTxHash && (
                    <Banner type="success">
                      <span style={{ wordWrap: "break-word", wordBreak: "break-all" }}>
                        {t<string>(`Your cross-chain asset transaction`)}
                      </span>
                      &nbsp;
                      <a
                        href={elrondTxQueryUrl}
                        target="_blank"
                        style={{ color: "#ff8812", fontStyle: "italic", textDecoration: "underline" }}
                      >
                        {t<string>(`{{ elrondTxHash }}`, {
                          replace: {
                            elrondTxHash
                          }
                        })}
                      ...
                      <span>
                        {t<string>(
                          `has been successfully sent. Please wait a few minutes to check your assets on Crust Mainnet-side`
                        )}
                      </span>
                      &nbsp;
                    </Banner>
              ...
              <Button.Group>
                <Button icon="paper-plane" isDisabled={isAmountError} label={t<string>("Submit")} onClick={submit} />
              </Button.Group>
            </Modal.Content>
          </Card>
        </Columar.Column>
